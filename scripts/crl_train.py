...
from recurrent_resnet.jax.recurrent_core import RecurrentResidualCore, ResidualStack
...
class SA_encoder(nn.Module):
    ...
    use_relu: int = 0
    encoder_type: str = "untied_resnet"
    recur_steps: int = 4
    recur_init_identity: int = 1
    recur_trunc_bptt: int = 0
    @nn.compact
...
class Actor(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, steps: Optional[jnp.ndarray] = None):
        if self.norm_type == "layer_norm":
            normalize = lambda x: nn.LayerNorm()(x)
        else:
            normalize = lambda x: x
...
if __name__ == "__main__":
    ...
    run_name = (
        ...
        f"enc:{args.encoder_type}_K:{args.recur_steps}_tbptt:{args.recur_trunc_bptt}_"
    ...
    # Network setup
    actor_encoder_type = args.encoder_type if args.recur_apply_to_actor else "untied_resnet"
    ...
    def multi_sample_actor_step(training_state, env, env_state, key, K, extra_fields):
        ...
        goal = env_state.obs[:, args.goal_start_idx:args.goal_end_idx]
    ...
    def update_actor_and_alpha(transitions, training_state, key):
        ...
        def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
            ...
            stds = jnp.exp(log_stds)
            x_ts = means + stds * jax.random.normal(action_key, shape=means.shape, dtype=means.dtype)
            action = nn.tanh(x_ts)
            ...
            sa_repr = sa_encoder.apply(sa_encoder_params, state, action, steps=critic_steps)
            g_repr = g_encoder.apply(g_encoder_params, goal, steps=critic_steps)
